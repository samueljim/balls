...
impl WeaponMenuLayout {
    pub fn new() -> Self {
        ...
        let menu_w = if is_mobile { sw * 0.92 } else { (480.0_f32).min(sw * 0.85) };
        let menu_h = if is_mobile { sh * 0.9 } else { (620.0_f32).min(sh * 0.85) };
        let menu_x = sw / 2.0 - menu_w / 2.0;
        let menu_y = sh / 2.0 - menu_h / 2.0;
        let header_h = 60.0;
        let content_y = menu_y + header_h + 10.0;
        let footer_h = 60.0;
        let content_h = menu_h - header_h - 10.0 - footer_h;
        let padding = if is_mobile { 8.0 } else { 12.0 };
        let item_h = if is_mobile { 52.0 } else { 58.0 };
        let cat_header_h = if is_mobile { 26.0 } else { 30.0 };
        let item_padding = 3.0;
        Self { menu_x, menu_y, menu_w, menu_h, header_h, content_y, content_h, padding, item_h, cat_header_h, item_padding, is_mobile }
    }
        ...
        for (i, cat) in categories.iter().enumerate() {
            if let Some(weapons) = by_category.get(cat) {
                ...
                if i < categories.len() - 1 {
                    h += 4.0; // space between categories
                }
            }
        }
        h
...
pub fn draw_hud(
    ...
    draw_text(
        "WASD move  Space jump  TAB weapons  E end turn  Scroll zoom  Right-drag pan",
        10.0,
        sh - 6.0,
        13.0,
...
fn draw_weapon_menu(selected_weapon: Weapon, scroll_offset: f32) {
    ...
    let title_size = if is_mobile { 22.0 } else { 26.0 };
    ...
    let clip_x = menu_x as i32;
    let clip_screen_y = (sh - (content_y + content_h)) as i32; // GL coords: bottom-left origin
    let clip_w = menu_w as i32;
    let clip_h = content_h as i32;
    unsafe {
        get_internal_gl().quad_gl.scissor(Some((clip_x, clip_screen_y, clip_w, clip_h)));
    }
    ...
    for cat in &categories {
        if let Some(weapons) = by_category.get(cat) {
            ...
            // Only draw if visible (optimisation, scissor handles clipping)
            if cat_y + cat_header_h > content_y - 10.0 && cat_y < content_y + content_h + 10.0 {
                ...
                let cat_size = if is_mobile { 13.0 } else { 15.0 };
                draw_text(
                    cat.name(),
                    menu_x + padding + 8.0,
                    cat_y + cat_header_h - 8.0,
                    cat_size,
            ...
            for w in weapons {
                ...
                // Only draw items that are in or near the visible area
                if item_y + item_h > content_y - 10.0 && item_y < content_y + content_h + 10.0 {
                    ...
                    // Selection border
                    if is_selected {
                        draw_rectangle_lines(item_x, item_y, item_w, item_h, 2.5, Color::new(0.3, 0.9, 0.5, 1.0));
                        // Glow effect
                    ...
                    // Weapon icon (larger)
                    let icon_size = if is_mobile { 28.0 } else { 32.0 };
                    let icon_x = item_x + 12.0;
                    draw_text(
                        w.icon(),
                        icon_x,
                    ...
                    // Weapon name
                    let name_x = icon_x + if is_mobile { 40.0 } else { 48.0 };
                    let name_size = if is_mobile { 16.0 } else { 18.0 };
                    draw_text(
                        w.name(),
                        name_x,
                        item_y + if is_mobile { 20.0 } else { 22.0 },
                        name_size,
                        if is_selected { 
                            Color::new(1.0, 1.0, 1.0, 1.0) 
                        } else { 
                    ...
                    // Weapon description
                    let desc_size = if is_mobile { 11.0 } else { 12.0 };
                    draw_text(
                        w.description(),
                        name_x,
                        item_y + if is_mobile { 38.0 } else { 42.0 },
                        desc_size,
                        Color::new(0.55, 0.6, 0.7, 0.95),
                    );
                    
                    // Stats badges (right side)
                    let stats_x = item_x + item_w - if is_mobile { 100.0 } else { 120.0 };
                    let badge_y = item_y + if is_mobile { 14.0 } else { 16.0 };
                    let badge_w = if is_mobile { 44.0 } else { 52.0 };
                    let badge_h = if is_mobile { 20.0 } else { 24.0 };
                    let badge_spacing = if is_mobile { 3.0 } else { 4.0 };
                    ...
                    if w.explosion_radius() > 0.0 {
                        draw_rectangle(
                            stats_x + badge_w + badge_spacing,
                            badge_y,
                            badge_w,
                            badge_h,
                            Color::new(0.2, 0.4, 0.7, 0.7),
                        );
                        let rad_text = format!("{:.0}", w.explosion_radius());
                        let rad_w = measure_text(&rad_text, None, desc_size as u16, 1.0).width;
                        draw_text(
                            &rad_text,
                            stats_x + badge_w + badge_spacing + badge_w / 2.0 - rad_w / 2.0,
                            badge_y + badge_h - 5.0,
                            desc_size,
                            WHITE,
                        );
    ...
    if max_scroll > 0.0 {
        ...
        let thumb_h = (track_h * visible_ratio).max(20.0);
    ...
    // Footer with hints
    let footer_y = menu_y + menu_h - 50.0;
    draw_rectangle(menu_x, footer_y, menu_w, 50.0, Color::new(0.1, 0.12, 0.16, 1.0));
    draw_rectangle(menu_x, footer_y, menu_w, 1.0, Color::new(0.3, 0.5, 0.7, 0.4));
    
    let hint = if is_mobile { 
        "Tap to select • Close with X" 
    } else { 
        "Click to select • Scroll to browse • TAB/ESC to close" 
    };
    let hint_size = if is_mobile { 13.0 } else { 14.0 };
